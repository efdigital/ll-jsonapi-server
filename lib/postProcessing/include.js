...
includePP._fillIncludeTree = (includeTree, request, callback) => {
  ...
  ****/
  ...
  Object.keys(map.primary).forEach(relation => {
    ...
    if (request.params.fields && request.params.fields[parts[0]]) {
      const urlJoiner2 = `&fields%5B${parts[0]}%5D%5B%5D=`
      ids += urlJoiner2 + request.params.fields[parts[0]].join(urlJoiner2);
    }
